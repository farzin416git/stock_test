...
from src.config import CANDIDATE_SEQ_LENS, BATCH_SIZE, LEARNING_RATE, DEVICE, RANDOM_SEED, EPOCHS, HORIZON
...
def train_model(X_train, y_train_reg, y_train_cls, X_valid, y_valid_reg, y_valid_cls, n_vars, seq_len):
    ...
    model = ParallelCNNLSTM(n_vars=n_vars, seq_len=seq_len, hidden_dim=128, horizon=HORIZON).to(DEVICE)
